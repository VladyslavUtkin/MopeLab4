...
def check(X, Y, B, n, m, norm=False):
    ...
    unimportance = [True if t < student_cr_table else False for t in ts]
    b_to_print = list(compress(B, unimportance))
    x2_i_names = list(
        compress(["", "x1", "x2", "x3", "x12", "x13", "x23", "x123", "x1^2", "x2^2", "x3^2"], unimportance))
    eq = " ".join(["".join(i) for i in zip(list(map(lambda x: "{:+.3f}".format(x), b_to_print)), x2_i_names)])
    print("*" * 100)
...
def linear(n, m):
    ...
    unimportance = [True if temp < student_cr_table else False for temp in student_t]
    b_to_print = list(compress(B, unimportance))
    x2_i_names = list(
        compress(["", "x1", "x2", "x3", "x12", "x13", "x23", "x123", "x1^2", "x2^2", "x3^2"], unimportance))
    eq = " ".join(["".join(i) for i in zip(list(map(lambda x: "{:+.3f}".format(x), b_to_print)), x2_i_names)])
